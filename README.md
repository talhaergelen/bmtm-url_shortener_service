...
[![CI/CD Pipeline](https://github.com/talhaergelen/bmtm-url_shortener_service/actions/workflows/ci.yml/badge.svg)](https://github.com/talhaergelen/bmtm-url_shortener_service/actions)
...
- 📈 Prometheus + Grafana — Gerçek zamanlı metrik izleme (6 panel)
- ☁️ LocalStack/S3 — İstatistiklerin buluta kaydedilmesi
- 🔭 OpenTelemetry + Jaeger — Distributed tracing (**Bonus**)
- ⎈ Helm chart — Kubernetes paketleme (**Bonus**)
- 📡 KEDA — Event-driven autoscaling (**Bonus**)
- 🔄 ArgoCD — GitOps CD pipeline (**Bonus**)
...
- `src/tracing.py` — OpenTelemetry distributed tracing (**Bonus**)
...
```bash
# Tüm servisleri başlat (app + localstack + prometheus + grafana + jaeger)
docker-compose up -d

# Servis adresleri:
# http://localhost:8000  → Uygulama
# http://localhost:4566  → LocalStack (Sahte AWS S3)
# http://localhost:9090  → Prometheus
# http://localhost:3000  → Grafana (admin/admin)
# http://localhost:16686 → Jaeger UI (Distributed Tracing) ← YENİ
...
### Yöntem 4: Helm Chart (Bonus)

```bash
# Helm ile kur
helm install url-shortener ./helm/url-shortener

# Özelleştirerek kur (örnek: 3 replika)
helm install url-shortener ./helm/url-shortener \
  --set replicaCount=3 \
...
# Sadece integration testler (Docker gerekli — Testcontainers)
...
├── docker-compose.yml           ← Tam stack (app+localstack+prometheus+grafana+jaeger)
...
│   ├── tracing.py               ← OpenTelemetry tracing ← BONUS
│   └── static/
...
│   │   └── test_aws.py          ← S3 mock testleri ← YENİ
│   ├── integration/
│   │   ├── test_api.py          ← API endpoint testleri
│   │   └── test_database.py     ← Testcontainers DB testleri
│   └── e2e/
│       └── test_ui.py           ← Playwright E2E testleri (5 senaryo)
├── postman/
│   └── collection.json          ← Newman ile çalışan koleksiyon (8 istek)
├── k8s/
│   ├── configmap.yaml           ← Kubernetes ortam ayarları
│   ├── deployment.yaml          ← Pod tanımı (2 replika)
│   ├── service.yaml             ← NodePort servisi
│   ├── keda-scaledobject.yaml   ← KEDA autoscaling ← BONUS
│   └── argocd-application.yaml  ← ArgoCD GitOps ← BONUS
...
├── .github/
│   └── workflows/
│       └── ci.yml               ← GitHub Actions (7 job: lint→test→postman→docker→deploy→smoke→e2e)
├── monitoring/
...
│   └── report.md                ← Performans sonuçları (p95=87ms)
└── docs/
    ├── architecture.png         ← Mimari diyagram ← YENİ
    ├── final-report.md          ← Final rapor taslağı
    ├── slides.md                ← Sunum slaytları
...
Test sayıları:
- Unit testler: ~20+ test (crud, shortener, aws)
- Integration testler: 8+ test (api + testcontainers)
- E2E testler: 5 Playwright senaryosu
- Postman: 8 istek, her birinde assertion'lar

---

## 🌟 Bonus Özellikler

| Özellik | Açıklama | Konum | Puan |
|---------|----------|-------|------|
| **Helm Chart** | K8s paketleme, `helm install` ile deploy | `helm/url-shortener/` | +5 |
| **KEDA Autoscaling** | Prometheus metriğine göre scale | `k8s/keda-scaledobject.yaml` | +5 |
| **ArgoCD GitOps** | Git'e push → otomatik deploy | `k8s/argocd-application.yaml` | +5 |
| **OpenTelemetry** | Distributed tracing, Jaeger UI | `src/tracing.py` | +5 |

**Toplam Bonus: +20 puan** (max +15 — yani tavan)
...
MIT License — Bkz. [LICENSE](LICENSE)
demo canli degisikligi eklendi
